package com.example.a8477_daniella_queiroz_androidprototype.database;
...
public class ListDatabase extends SQLiteOpenHelper {
    ...
    private static final String TABLE_NAME = "productList";
    ...
    //create sql statements initial version will auto increment the number of the recipe
    ...
    //it's just going to be executed if the database exists
    @Override
    ...
    //will choose a picture from drawble
    ...
    public boolean update(Long id, String recipeName, String ingredients, Long duration) {
        //create a instance sqlite database
        SQLiteDatabase db = this.getWritableDatabase();

        //it's going to pass the value
        ContentValues contentValues = new ContentValues();
...
//get the recipe by the id
    ...
    public boolean rateRecipe(Long id, Integer stars){
        ...
        db.execSQL(UPDATE_RECIPE_VOTES, new String[ ]{
                stars.toString(), id.toString() });
        return true;
    }
}
